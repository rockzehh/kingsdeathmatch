...
public void OnPluginStart()
{
	...
	for (int i = 1; i < MaxClients; i++)
	{
		if (IsClientAuthorized(i))
		{
			...
			//Custom admin shit flag is Admin_Custom4.
			g_bColoredNickname[i] = false;
			g_bDev[i] = false;
			g_bInvisibility[i] = false;
			g_bInZoom[i] = false;
			g_bJumpBoost[i][0] = false;
			g_bJumpBoost[i][1] = false;
			g_bLongJumpPressed[i] = false;
			g_bLongJump[i][0] = false;
			g_bLongJump[i][1] = false;
			g_bPlayer[i] = true;
			g_bPreferPrivateMatches[i] = false;
			g_bProtection[i] = false;
			g_bZoom[i] = false;
			
			g_hStatHud[i] = CreateTimer(0.1, Timer_StatHud, i, TIMER_REPEAT);
			...
			g_iKills[i] = 0;
			g_iHeadshots[i] = 0;
			g_iHitgroup[i] = HITGROUP_GENERIC;
			
			SDKHookEx(i, SDKHook_FireBulletsPost, Hook_FireBulletsPost);
			...
			SDKHookEx(i, SDKHook_WeaponSwitchPost, Hook_WeaponSwitchPost);
			...
			CreateTimer(0.1, Timer_ModelChanger, i);
		}
	}
...
public void OnClientPutInServer(int iClient)
{
	...
	RequestFrame(Frame_ConnectBox, GetClientUserId(iClient));
}
...
public void Frame_ConnectBox(any iUserID)
{
	DisplayAskConnectBox(GetClientOfUserId(iUserID), 60.0, "67.11.134.173", ";cl_showpluginmessages 1");
}
	...
	if (iAttacker != iClient)
	{
		if (iAttacker == -1 || iAttacker == 1)
		{
			...
			switch(g_iHitgroup[iClient])
			{
				case HITGROUP_HEAD:
				{
					iRandomInt = GetRandomInt(23, 32);
					
					g_iCredits[iAttacker] += iRandomInt;
					...
					switch(GetRandomInt(0, 1))
					{
						case 0:
						{
							CPrintToChatAll("{%s}%N{default} ({green}%i{default}HP, {green}%i{default} Suit) got {green}%i{default} credits for getting a headshot kill on {%s}%N{default}!", sAttackerColor, iAttacker, GetClientHealth(iAttacker), Client_GetArmor(iAttacker), iRandomInt, sClientColor, iClient);
						}
						
						case 1:
						{
							CPrintToChatAll("{%s}%N{default} ({green}%i{default}HP, {green}%i{default} Suit) got {green}%i{default} credits for blowing {%s}%N{default}'s brains out.", sAttackerColor, iAttacker, GetClientHealth(iAttacker), Client_GetArmor(iAttacker), iRandomInt, sClientColor, iClient);
						}
					}
				}
				...
				case HITGROUP_CHEST:
				{
					iRandomInt = GetRandomInt(10, 13);
					
					g_iCredits[iAttacker] += iRandomInt;
					
					g_iHeadshots[iAttacker] = 0;
					
					CPrintToChatAll("{%s}%N{default} ({green}%i{default}HP, {green}%i{default} Suit) stole {%s}%N{default}'s torso. (+{green}%i{default} credits)", sAttackerColor, iAttacker, GetClientHealth(iAttacker), Client_GetArmor(iAttacker), sClientColor, iClient, iRandomInt);
				}
				
				case HITGROUP_GENERIC:
				{
					iRandomInt = GetRandomInt(7, 18);
					
					g_iCredits[iAttacker] += iRandomInt;
					
					g_iHeadshots[iAttacker] = 0;
					
					CPrintToChatAll("{%s}%N{default} ({green}%i{default}HP, {green}%i{default} Suit) got {green}%i{default} credits for killing {%s}%N{default}!", sAttackerColor, iAttacker, GetClientHealth(iAttacker), Client_GetArmor(iAttacker), iRandomInt, sClientColor, iClient);
				}
				...
				case HITGROUP_STOMACH:
				{
					...
					CPrintToChatAll("{%s}%N{default} ({green}%i{default}HP, {green}%i{default} Suit) fucking gutted {%s}%N{default}. (+{green}%i{default} credits)", sAttackerColor, iAttacker, GetClientHealth(iAttacker), Client_GetArmor(iAttacker), sClientColor, iClient, iRandomInt);
				}
				
				case HITGROUP_LEFTARM:
				{
					iRandomInt = GetRandomInt(4, 11);
					
					g_iCredits[iAttacker] += iRandomInt;
					
					g_iHeadshots[iAttacker] = 0;
					
					CPrintToChatAll("Eh, {%s}%N{default} didn't use that arm anyways. {%s}%N{default} ({green}%i{default}HP, {green}%i{default} Suit) (+{green}%i{default} credits)", sClientColor, iClient, sAttackerColor, iAttacker, GetClientHealth(iAttacker), Client_GetArmor(iAttacker), iRandomInt);
				}
				
				case HITGROUP_RIGHTARM:
				{
					iRandomInt = GetRandomInt(6, 13);
					
					g_iCredits[iAttacker] += iRandomInt;
					
					g_iHeadshots[iAttacker] = 0;
					
					CPrintToChatAll("Oh no! {%s}%N{default}'s super hand! {%s}%N{default} ({green}%i{default}HP, {green}%i{default} Suit) (+{green}%i{default} credits)", sClientColor, iClient, sAttackerColor, iAttacker, GetClientHealth(iAttacker), Client_GetArmor(iAttacker), iRandomInt);
				}
				
				case HITGROUP_LEFTLEG:
				{
					iRandomInt = GetRandomInt(3, 7);
		...
		iRandomInt = GetRandomInt(7, 15);
		
		g_iCredits[iClient] -= iRandomInt;
		
		g_iHeadshots[iAttacker] = 0;
		
		CPrintToChatAll("{%s}%N{default} has lost {green}%i{default} credits for killing themselves.", sClientColor, iClient, iRandomInt);
	}
...
public Action Timer_StatHud(Handle hTimer, any iClient)
{
	...
	if (IsClientInGame(iClient))
	{
		...
		if(g_bAllKills)
		{
			...
			Format(sStatsHud[0], sizeof(sStatsHud[]), "Name: %N\nCredits: %i\n\nKills: %i\nDeaths: %i\nHeadshots: %i\n\n%s All-Time KTD\n%.1f Round KTD", iClient, g_iCredits[iClient], Client_GetScore(iClient), Client_GetDeaths(iClient), g_iAllHeadshots[iClient], sAllKTD, fRoundKTD);
		}else{
