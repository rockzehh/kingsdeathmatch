...
public Action OnPlayerRunCmd(int iClient, int &iButtons, int &iImpulse, float fVel[3], float fAngles[3], int &iWeapon, int &iSubtype, int &iCmdnum, int &iTickcount, int &iSeed, int iMouse[2])
{
	...
	if(StrEqual(sWeaponClass, "weapon_smg") && (iButtons & IN_ATTACK2))
	{
...
public Action Command_JumpBoost(int iClient, int iArgs)
{
	...
	CPrintToChat(iClient, "[{red}KINGS{default}] You have enabled the {green}Jump Boost Module{default} for the first time.");
...
public Action Command_LongJump(int iClient, int iArgs)
{
	...
	CPrintToChat(iClient, "[{red}KINGS{default}] You have enabled the {green}Long Jump Module{default} for the first time.");
...
{
	char sCountry[4], sIP[64], sName[MAX_NAME_LENGTH];
