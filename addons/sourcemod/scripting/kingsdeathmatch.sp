...
#define PLUGIN_VERSION "1.3.0-b4"
...
public void OnPluginStart()
{
	...
	g_cvUpgradePriceDistort = CreateConVar("kdm_invisible_price", "500", "The amount of credits you need to pay to use the invisible effect.");
	...
	RegConsoleCmd("sm_invisible", Command_Distort, "Makes your player model invisible.");
...
public Action OnPlayerRunCmd(int iClient, int &iButtons, int &iImpulse, float fVel[3], float fAngles[3], int &iWeapon, int &iSubtype, int &iCmdnum, int &iTickcount, int &iSeed, int iMouse[2])
{
	...
	float fDistance, fSpawnPoint[3], fVelocity[3];
	
	GetClientAbsOrigin(iClient, fSpawnPoint);
	
	fDistance = GetVectorDistance(fSpawnPoint, g_fSpawnPoint[iClient]);
	...
	if(g_bProtection[iClient])
	{
		if(fDistance > 0.1)
		{
			...
			SetEntityRenderFx(iClient, g_bDistort[iClient] ? RENDERFX_DISTORT : RENDERFX_NONE);
...
public Action Command_Credits(int iClient, int iArgs)
{
	...
	if(g_bMenu)
	{
		...
		Format(sDescription, sizeof(sDescription), "Invisibility | %i Credits", g_iUpgrade_Prices[0]);
	...
	if (iAttacker != iClient)
	{
		if (iAttacker == -1 || iAttacker == 0)
		{
			...
			if(bHeadshot)
			{
				...
				CPrintToChatAll("{green}%N{default} got {green}16{default} credits for getting a headshot kill on {green}%N{default}!", iAttacker, iClient);
			}else{
